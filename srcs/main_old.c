...
void	*ft_malloc_old(size_t size)
{
	t_zone 	*new;
	t_zone 	*tmp;
	char 	*type;
	...
	if (!g_zone)
		if ((void*)(g_zone = (t_zone**)mmap(0, sizeof(t_zone*),
	...
	while (tmp && ft_strcmp(type, "LARGE"))
	{
		if (tmp->type == type)
		{
			if ((unsigned long)tmp->current + sizeof(t_alloc) + size
				< (unsigned long)tmp + length)
			{
				ft_putendl("Option 2");
				tmp->current->next = alloc_create(tmp->current->start
					+ tmp->current->length + 1, size);
